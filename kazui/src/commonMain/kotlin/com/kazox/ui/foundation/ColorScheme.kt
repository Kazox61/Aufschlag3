...
/**
 * Built-in color palettes for KazUI, matching shadcn/ui v4 exactly.
 *
 ...
 * KazTheme(colors = KazPalettes.ZincDark) { ... }
 *
 * // Apply an accent color on top
 * val colors = KazPalettes.ZincDark.withAccent(KazAccentDark.Blue)
 * KazTheme(colors = colors) { ... }
 * ```
 *
 * All hex values are taken directly from the shadcn/ui v4 source and
 * converted from HSL to sRGB hex, except for the `destructive` and
 * `success` families, which deviate so that every foreground/background
 * pair (including hover and pressed states) meets the WCAG 4.5:1 contrast
 * ratio for normal text.
 */
public object KazPalettes {
    /** Zinc base palette, light variant. */
    public val ZincLight: KazColors =
        KazColors(
    ...
    public val SlateLight: KazColors =
        KazColors(
    ...
    public val SlateDark: KazColors =
        KazColors(
    ...
    public val StoneLight: KazColors =
        KazColors(
    ...
    public val StoneDark: KazColors =
        KazColors(
    ...
    public val GrayLight: KazColors =
        KazColors(
    ...
    public val NeutralLight: KazColors =
        KazColors(
    ...
    public val NeutralDark: KazColors =
        KazColors(
 ...
 *
 * @see KazAccent for light-mode accent presets.
 * @see KazAccentDark for dark-mode accent presets.
 * @see withAccent to apply an accent to a [KazColors] instance.
 */
public data class KazAccentColor(
    val primary: Color,
    val onPrimary: Color,
    val ring: Color,
 ...
 * ### Usage
 * ```
 * val colors = KazPalettes.ZincLight.withAccent(KazAccent.Blue)
 * KazTheme(colors = colors) { ... }
 * ```
 */
public object KazAccent {
    /** Vibrant red accent. red-600 -> hover red-700, press red-800 */
    public val Red: KazAccentColor =
        KazAccentColor(
    ...
    public val Rose: KazAccentColor =
        KazAccentColor(
    ...
    public val Orange: KazAccentColor =
        KazAccentColor(
    ...
    public val Green: KazAccentColor =
        KazAccentColor(
    ...
    public val Blue: KazAccentColor =
        KazAccentColor(
    ...
    public val Yellow: KazAccentColor =
        KazAccentColor(
    ...
    public val Violet: KazAccentColor =
        KazAccentColor(
...
/**
 * Applies an [accent] color override to this [KazColors] instance.
 *
 ...
 * ```
 *
 * val lightBlue = KazPalettes.SlateLight.withAccent(KazAccent.Blue)
 *
 *
 * val darkViolet = KazPalettes.SlateDark.withAccent(KazAccentDark.Violet)
 * ```
 *
 * @param accent The accent color to apply.
 * @return A new [KazColors] with the accent's primary tokens applied.
 */
public fun KazColors.withAccent(accent: KazAccentColor): KazColors =
    copy(
 ...
 *
 * Each entry maps to a light/dark pair in [KazPalettes].
 * Use [resolve] to obtain the corresponding [KazColors].
 *
 * ### Usage
 * ```
 * val colors = KazPalette.Zinc.resolve(isDark = true)
 * KazTheme(colors = colors) { ... }
 * ```
 */
public enum class KazPalette(
    ...
    /** Resolves to a [KazColors] for the given dark-mode flag. */
    public fun resolve(isDark: Boolean): KazColors =
        when (this) {
            Zinc -> if (isDark) KazPalettes.ZincDark else KazPalettes.ZincLight
            Slate -> if (isDark) KazPalettes.SlateDark else KazPalettes.SlateLight
            Stone -> if (isDark) KazPalettes.StoneDark else KazPalettes.StoneLight
            Gray -> if (isDark) KazPalettes.GrayDark else KazPalettes.GrayLight
            Neutral -> if (isDark) KazPalettes.NeutralDark else KazPalettes.NeutralLight
        }
}
 ...
 * All other entries map to light/dark pairs in [KazAccent] / [KazAccentDark].
 *
 * ### Usage
 * ```
 * val base = KazPalette.Zinc.resolve(isDark = true)
 * val colors = KazAccentPreset.Blue.applyTo(base, isDark = true)
 * KazTheme(colors = colors) { ... }
 * ```
 */
public enum class KazAccentPreset(
    public val label: String,
) {
    Default("Default"),
    Blue("Blue"),
    ...
    /**
     * Applies this accent to a base [KazColors].
    ...
    public fun applyTo(
        ...
        val accent =
            if (isDark) {
                when (this) {
                    Red -> KazAccentDark.Red
                    Rose -> KazAccentDark.Rose
                    Orange -> KazAccentDark.Orange
                    Green -> KazAccentDark.Green
                    Blue -> KazAccentDark.Blue
                    Yellow -> KazAccentDark.Yellow
                    Violet -> KazAccentDark.Violet
                ...
                when (this) {
                    Red -> KazAccent.Red
                    Rose -> KazAccent.Rose
                    Orange -> KazAccent.Orange
                    Green -> KazAccent.Green
                    Blue -> KazAccent.Blue
                    Yellow -> KazAccent.Yellow
                    Violet -> KazAccent.Violet
                    Default -> error("unreachable")
                }
            }
