...
/**
 * Single point for plan/feature and club-status gating — call
...
class EntitlementService {
    ...
    fun directoryHiddenStatuses(): Set<ClubStatus> = setOf(ClubStatus.ARCHIVED)

    /** SUSPENDED/ARCHIVED clubs don't accept new Mitgliedsanträge. */
    fun requireAcceptingApplications(club: ClubRow) {
        if (club.status == ClubStatus.SUSPENDED || club.status == ClubStatus.ARCHIVED) {
